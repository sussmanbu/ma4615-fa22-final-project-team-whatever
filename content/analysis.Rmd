...
```{r echo=FALSE}
ggplot(nd_regions, aes(x = reorder(Country, LowBirthWT), y = LowBirthWT,  fill=Country)) + 
    coord_flip() +
...
#similar plot, for Low Birthweight
 #nd_regions%>%ggplot() + geom_bar(aes(x = reorder(Country, LowBirthWT), y = LowBirthWT, fill = Country), stat = 'identity') + coord_flip() + labs(title = '% of Low Birth Weights by World Region', x = 'World Region', y = '% of Low Birth Weights')
```
...
This is our first attempt at exploring a variable from the nutrition data set with income per capita. Here we are able to see that as income increases, exclusive breast feeding for children's nutrition decreases. Exclusive breast feeding, according to thi s data, is described as the
...
=======
>>>>>>> 43751933b22e1bc9cde93582dcf514124b065132
```{r echo=FALSE}

...
```{r}
#set your own working directory; how to make it work for everybody?
path <- "/Users/francescobenatti/Library/Mobile Documents/com~apple~CloudDocs/BU/MA615 Data Science with R/ma4615-fa22-final-project-team-whatever"
setwd(path)

#problem: doesn't load the data...
load("/dataset/income_mortality.RData")
load("/static/load_and_clean_data.R")
...
income_mortality %>% filter(Year == "1950" | Year == "1975" | Year == "2000" | Year == "2020") %>% ggplot(aes(x = Income, y = Mortality, color = Income)) + geom_point(size = 0.3) +
  ...
  scale_x_log10() +
  labs(x = "Income Per Capita (PPP)", y = "Under 5 Mortality Rate", title = "Disparity in Under 5 Mortality Rates has decreased over time")
```

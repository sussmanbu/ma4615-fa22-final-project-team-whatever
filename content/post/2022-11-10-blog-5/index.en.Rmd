...
Here are a few steps we took or might need to take in the future to use this Mortality data set: 
  ...
  2. As per the nutrition dataset, we have already done most of the cleaning in the load and clean data script: we filtered and renamed the columns, coverted the values from characters to numbers, and selected only the median estimate of mortality values for each country (the data provided three estimates for each country, to give a confidence interval: low, median and high).
  3. Values were given as deaths per thousand, so we transformed it to percentages by dividing the values by 10.


Essentially, the question we want to address is: Why do some countries have higher children's (under 5) mortality rates than others? What are some influential factors that cause these differences in mortality? Possible factors we will investigate are: Nutrition, GDP, Sanitation, Education levels, Diseases like HIV/AIDS, Diarrhea, Malaria etc., and Violence levels). As per the first two data sets, most of these variables can be extracted from different data sets provided by UNICEF data website. 
